...
function playMusic(songName) {
  ...
  for(let i = 1; i < 101; i++) {
    ...
    try {
      document.getElementById('song-time-bar-touch-'+i).removeEventListener('click');
    } catch(err) {
      //No event listener
    }

    document.getElementById('song-time-bar-touch-'+i).addEventListener('click', () => {

      audio.currentTime = Math.floor((songLength/100) * i);
